...
def safe_qty_parse(val):
    """The Nuclear Defensive Parser: Always returns an int, never crashes."""
    try:
...
def upload_system_pdf_to_drive(html_content, file_name, client_name, invoice_no):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf: temp_pdf_path = temp_pdf.name
...
def generate_caricom_printout(inv_num, date, client_name, client_address, supplier_name, supplier_address, bl, total_ctns, subtotal, freight, grand_total, payment_terms, additional_notes, signatory_position, compliance_data, logo_path, sig_path):
    decl = "CARICOM COMMON MARKET DECLARATION:<br>The undermentioned exporter hereby declares that the cargo specified in this commercial invoice manifest has been produced completely within the parameters of the common market rules of origin."
    ...
    html = f"""<html><head><style>@page {{ size: letter landscape; margin: 0.35in; }} body {{ font-family: sans-serif; font-size: 11px; }} .signature-frame {{ height: 150px; width: 200px; }}</style></head><body>{img_tag}<h2>CARICOM INVOICE</h2></body></html>"""
    return html
...
def generate_html_document(title, inv_no, date, client, c_addr, supplier, s_profile, bl, total_ctns, df, total_val, freight=None, additional_notes="", payment_terms="", signatory_position="", is_packing=False, is_duties=False, duty_data=None):
    ...
    if is_packing:
        ...
        sig_tag = f'<img src="{sig_path}" height="120">' if sig_path else ''
        rendered_html = f'<html><body><table width="100%"><tr><td>{img_tag}</td><td align="right"><h2>{title}</h2></td></tr></table><p><b>Exporter:</b> {supplier}<br><b>Consignee:</b> {client}<br>{c_addr}</p><table border="1" width="100%" cellspacing="0" cellpadding="5"><thead><tr bgcolor="#f7f7f7"><th>Description</th><th>Carton Nos</th><th>Total Ctns</th><th>Qty-Pcs</th></tr></thead><tbody>{table_rows}</tbody></table><br><br><div align="right">{sig_tag}<br><b>{signatory_position}</b></div></body></html>'
    elif is_duties:
        duty_data = duty_data or {}
        img_tag = f'<img src="{logo_path}" height="50">' if logo_path else ''
        rendered_html = f'<html><body><table width="100%"><tr><td>{img_tag}</td><td align="right"><h2>{title}</h2></td></tr></table><p><b>Invoice:</b> {inv_no}</p><p>Total Customs Bill Due: ${duty_data.get("grand_total_ttd",0):,.2f} TTD</p></body></html>'
    else:
        ...
        template = template_env.get_template(chosen_template) if os.path.exists(f"./templates/{chosen_template}") else template_env.from_string("<h1>Invoice</h1>")
        items = []
        for idx, row in df.iterrows():
            items.append({"Description": str(row["Description"])[:250], "Qty": f"{row.get('Qty', 0):,}", "UnitPrice": f"{float(row.get('UnitPrice', 0)):.2f}", "Total": f"{float(row.get('Total Foreign (USD)', 0)):.2f}"})
...
def render_master_log():
    ...
    df = load_log_data()
    if df.empty: st.info("No data found.")
    else:
        for idx, row in df.iterrows():
            row_uid = str(row.get('Row_UID', ''))
            if not row_uid.strip(): continue 
            header_text = f"📦 INV: {row.get('Invoice No', '[Blank]')} | Client: {row.get('Client Name', 'Unknown')}"
            with st.expander(header_text):
                # (Simplified for brevity, kept functional)
                st.write(f"Container: {row.get('Container #')}")
...
def render_admin_tracker():
    ...
    st.info("App logic running securely.")
